package com.fjmob.ponto.persistence;

import android.content.ContentValues;
import android.content.Context;
import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;

import com.fjmob.ponto.entity.Falta;
import com.fjmob.ponto.entity.Historico;

import java.text.ParseException;
import java.text.SimpleDateFormat;
import java.util.ArrayList;
import java.util.Date;
import java.util.List;

public class FaltaDAO {


    public static final String NOME_TABELA = "Falta";
    public static final String COLUNA_ID = "id";
    public static final String COLUNA_DATA_GRAVACAO = "dataGravacao";
    public static final String COLUNA_COMENTARIO = "comentario";


    public static final String SCRIPT_CRIACAO_TABELA = "CREATE TABLE " + NOME_TABELA + "("
            + COLUNA_ID + " INTEGER PRIMARY KEY," +
    		COLUNA_DATA_GRAVACAO + " TEXT, " + COLUNA_COMENTARIO + " TEXT)";

    public static final String SCRIPT_DELECAO_TABELA =  "DROP TABLE IF EXISTS " + NOME_TABELA;


    private SQLiteDatabase dataBase = null;


    private static FaltaDAO instance;

    public static FaltaDAO getInstance(Context context) {
        if(instance == null)
            instance = new FaltaDAO(context);
        return instance;
    }

    private FaltaDAO(Context context) {
        PersistenceHelper persistenceHelper = PersistenceHelper.getInstance(context);
        dataBase = persistenceHelper.getWritableDatabase();
    }
 
    public void salvar(Falta falta) {
        ContentValues values = gerarContentValuesHistorico(falta);
        dataBase.insert(NOME_TABELA, null, values);
    }
 
    public List<Falta> recuperarTodos() {
        String queryReturnAll = "SELECT * FROM " + NOME_TABELA;
        Cursor cursor = dataBase.rawQuery(queryReturnAll, null);
        List<Falta> faltas = construirHistoricoPorCursor(cursor);
 
        return faltas;
    }

    public Falta recuperarPorId(int id) {
    	String queryReturnAll = "SELECT * FROM " + NOME_TABELA + " WHERE " + COLUNA_ID + "= " + id;
    	Cursor cursor = dataBase.rawQuery(queryReturnAll, null);
    	List<Falta> faltas = construirHistoricoPorCursor(cursor);

    	if(faltas.size() > 0) {
    		return faltas.get(0);
    	}
    	return null;
    }
    
    public List<Falta> recuperarTodosOrdenados() {
        String queryReturnAll = "SELECT * FROM " + NOME_TABELA + " ORDER BY  "+COLUNA_ID +  " DESC";
        Cursor cursor = dataBase.rawQuery(queryReturnAll, null);
        List<Falta> faltas = construirHistoricoPorCursor(cursor);
 
        return faltas;
    }
 
    public void deletar(Falta falta) {
 
        String[] valoresParaSubstituir = {
                String.valueOf(falta.getId())
        };
 
        dataBase.delete(NOME_TABELA, COLUNA_ID + " =  ?", valoresParaSubstituir);
    }
 
    public void editar(Falta falta) {
        ContentValues valores = gerarContentValuesHistorico(falta);
 
        String[] valoresParaSubstituir = {
                String.valueOf(falta.getId())
        };
 
        dataBase.update(NOME_TABELA, valores, COLUNA_ID + " = ?", valoresParaSubstituir);
    }
 
    public void fecharConexao() {
        if(dataBase != null && dataBase.isOpen())
            dataBase.close(); 
    }
 
 
    private List<Falta> construirHistoricoPorCursor(Cursor cursor) {
        List<Falta> faltas = new ArrayList<Falta>();
        if(cursor == null)
            return faltas;
         
        try {
 
            if (cursor.moveToFirst()) {
                do {
 
                    int indexID = cursor.getColumnIndex(COLUNA_ID);
                    int indexDataGravacao = cursor.getColumnIndex(COLUNA_DATA_GRAVACAO);
                    int indexComentario = cursor.getColumnIndex(COLUNA_COMENTARIO);
 
                    int id = cursor.getInt(indexID);
                    String dataGravacaoTemp = cursor.getString(indexDataGravacao);
                    Date dataGravacao = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(dataGravacaoTemp);
                    String comentario = cursor.getString(indexComentario);

                    Falta faltaVO = new Falta(id, dataGravacao, comentario);

                    faltas.add(faltaVO);
 
                } while (cursor.moveToNext());
            }
             
        } catch (ParseException e) {
        	cursor.close();
		} finally {
            cursor.close();
        }
        return faltas;
    }
 
    private ContentValues gerarContentValuesHistorico(Falta falta) {
        ContentValues values = new ContentValues();
        values.put(COLUNA_DATA_GRAVACAO, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(falta.getDataGravacao()));
        values.put(COLUNA_COMENTARIO, falta.getComentario());
        
        return values;
    }

	public void deletarTodos() {
		dataBase.delete(NOME_TABELA, null, null);
	}
}
